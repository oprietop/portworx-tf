...
resource "google_compute_instance" "k8s_master" {
  ...
  attached_disk {
    source = "${google_compute_disk.px-master-disk.self_link}"
  }
...
resource "null_resource" "post_install" {
  provisioner "file" {
    connection {
      host        = "${google_compute_instance.k8s_master.network_interface.0.access_config.0.assigned_nat_ip}"
      type        = "ssh"
      user        = "root"
      private_key = "${file("${var.private_key_path}")}"
      agent       = false
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "chmod +x /root/deploy/run.sh && /root/deploy/run.sh",
    ]
  }
